...
router.post('/file', protect, uploadFileMiddleware.single('file'), async (req, res) => {
    try {
        if (!req.file) {
...
router.post('/', protect, videoUpload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ success: false, message: 'No file provided' });
        }
        ...
        if (isImage) {
            ...
            result = await uploadFile(req.file, 'chat/uploads');
        }
